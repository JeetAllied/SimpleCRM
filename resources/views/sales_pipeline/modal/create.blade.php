<form class="form-horizontal ng-pristine ng-valid ajaxFormSubmit" action="{{ route('sales-pipelines.store') }}" method="post">
    ...
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title">Add Sales Pipeline</h5>
        ...
        <div class="modal-body">
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="title" name="title" placeholder="Enter Sales Pipeline Title">
            </div>
            <div class="mb-3">
                ...
                <select name="opportunity_id" class="form-control select2" id="opportunity_id">
                    ...
                    @foreach($opportunities as $opportunity)
                        <option value="{{ $opportunity->id }}"> {{ $opportunity->opportunity_name }} </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="sales_pipeline_stage_id" class="form-control select2" id="sales_pipeline_stage_id">
                    ...
                    @foreach($salesPipelineStages as $salesPipelineStage)
                        <option value="{{ $salesPipelineStage->id }}"> {{ $salesPipelineStage->sales_pipeline_stage_name }} </option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                <label for="probability" class="form-label">Probability <span class="text-danger">*</span></label>
                <input type="number" min="0" class="form-control" id="probability" name="probability" placeholder="Enter Probability Value">
            </div>
