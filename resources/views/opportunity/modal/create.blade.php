<form class="form-horizontal ng-pristine ng-valid" action="{{ route('opportunities.store') }}" method="post" onsubmit="return validateOpportunity()">
    @csrf
            ...
            <div class="mb-3">
                ...
                <select name="lead_id" class="form-control select2" id="lead_id">
                    ...
                    @foreach($leads as $lead)
                        <option value="{{ $lead->id }}"> {{ $lead->lead_title }} </option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                <label for="expected_value" class="form-label">Expected Value <span class="text-danger">*</span></label>
                <input type="number" min="0" class="form-control" id="expected_value" name="expected_value" placeholder="Enter expected Value">
            </div>
            <div class="mb-3">
                ...
                <select name="opportunity_stage_id" class="form-control select2" id="opportunity_stage_id">
                    ...
                    @foreach($opportunityStages as $opportunityStage)
                        <option value="{{ $opportunityStage->id }}"> {{ $opportunityStage->opportunity_stage_name }} </option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                <label for="expected_close_date" class="form-label">Expected Close Date <span class="text-danger">*</span></label><br>
                <input type="text" class="form-control" id="expected_close_date" name="expected_close_date" placeholder="Select Expected Close Date">
            </div>
